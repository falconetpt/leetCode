...
public class TeemoAttacking {
    public int findPoisonedDuration(int[] timeSeries, int duration) {
        if (timeSeries == null || timeSeries.length == 0) {
            ...
            return IntStream.range( 1, timeSeries.length )
                    .map( i -> Math.min( timeSeries[i-1] + duration, timeSeries[i]) - timeSeries[i-1] )
                    .sum() + duration;
        }
    }
     ...
     * towards Ashe and the poisoning time duration per Teemo's attacking,
     ...
     * And at time point 4, Teemo attacks Ashe again, and causes
     * Ashe to be in poisoned status for another 2 seconds.
